...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            self.epsilon = math.exp(-0.001*self.trial_count)
            self.alpha = 0.5
            self.trial_count += 1
        return None
    ...
    def choose_action(self, state):
        ...
        if not self.learning or random.random() < self.epsilon:
            ...
            allkeys = [k for k, v in self.Q[state].items() if v == maxq]
            # for k,v in self.Q[self.state].items():
            #     if not maxq or v > maxq:
            #         action = k
            #         maxq = v
            action = random.choice(allkeys)
